...
#ifndef TEST_ALGO_SORT_MERGESORT_H_
#define TEST_ALGO_SORT_MERGESORT_H_
...
void test_mergesort(size_t n)
{
     ...
     * Test Sort
     */
    algo::sort<T, std::less_equal, algo::MergeSort>(v);
    for (size_t i = 0; i < v.size()-1; ++i) {
        REQUIRE(v[i] <= v[i+1]);                 /* sort in decreasing order */
    }

    algo::sort<T, std::greater_equal, algo::MergeSort>(v);
    for (size_t i = 0; i < v.size()-1; ++i) {
    ...
    std::vector<size_t> ltix =
        algo::argsort<T, std::less_equal, algo::MergeSort>(v);
    for (size_t i = 0; i < v.size()-1; ++i) {
        REQUIRE(v[ltix[i]] <= v[ltix[i+1]]);     /* sort in decreasing order */
    }

    std::vector<size_t> gtix =
        algo::argsort<T, std::greater_equal, algo::MergeSort>(v);
    for (size_t i = 0; i < v.size()-1; ++i) {
        REQUIRE(v[gtix[i]] >= v[gtix[i+1]]);     /* sort in increasing order */
    }
}

#endif /* TEST_ALGO_SORT_MERGESORT_H_ */
