...
class Graph:
    """
    Graph represents a undirected graph in adjacency list representation.
    """
    ...
    def adj(self, v):
        """ Iterate over the vertex adjacency list in lifo order """
        assert self.is_valid(v), "invalid vertex"
...
def main (argv):
    ...
    with open('../../data/mediumG.txt') as stream:
        graph1 = Graph.from_stream(stream)
    print(graph1)
    draw(graph1)

    # Create a graph from another graph
    graph2 = Graph.from_graph(graph1)
    print(graph2)
    draw(graph2)
