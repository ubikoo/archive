...
{
    { /* test abs */
        core::math::vec2<Type> a, abs_a;

        a = core::math::vec2<Type>(-1,0);
        abs_a = core::math::abs(a);
        for (size_t i = 0; i < a.length(); i++) {
            REQUIRE(core::math::isequal(abs_a(i), std::fabs(a(i))));
        }

        a = core::math::vec2<Type>(-1,1);
        abs_a = abs(a);
        for (size_t i = 0; i < a.length(); i++) {
            REQUIRE(core::math::isequal(abs_a(i), std::fabs(a(i))));
        }
    ...
    { /* test sign */
        core::math::vec2<Type> a, sign_a;

        a = core::math::vec2<Type>(-2,-3);
        sign_a = sign(a);
        for (size_t i = 0; i < a.length(); i++) {
    ...
    { /* test floor */
        core::math::vec2<Type> a, floor_a;

        a = core::math::vec2<Type>(-0.8, -0.4);
        floor_a = floor(a);
        for (size_t i = 0; i < a.length(); i++) {
    ...
    { /* test round */
        core::math::vec2<Type> a, round_a;

        a = core::math::vec2<Type>(-1.3, -1.6);
        round_a = round(a);
        for (size_t i = 0; i < a.length(); i++) {
    ...
    { /* test clamp */
        ...
        core::math::vec2<Type> a, clamp_a;
        Type lo = -1.0;
        Type hi =  0.0;
        a = core::math::vec2<Type>(-1.5, -0.5);
    ...
    { /* test lerp */
        ...
        auto lerp_fun = [&] (
            const core::math::vec2<Type> &lo,
            const core::math::vec2<Type> &hi,
            const Type alpha) -> core::math::vec2<Type> {
            return (lo*(one - alpha) + hi*alpha);
        };

        core::math::vec2<Type> lo(-1.0);
        core::math::vec2<Type> hi( 0.0);
        Type alpha = 0.0;
        while (alpha < 1.0) {
            core::math::vec2<Type> a = lerp(lo,  hi, alpha);
            core::math::vec2<Type> b = lerp_fun(lo,  hi, alpha);
            core::math::vec2<Type> e = (a - b);
            for (size_t i = 0; i < a.length(); ++i) {
        ...
        lo = core::math::vec2<Type>(0.0);
        hi = core::math::vec2<Type>(1.0);
        alpha = 0.0;
        while (alpha < 1.0) {
            core::math::vec2<Type> a = lerp(lo,  hi, alpha);
            core::math::vec2<Type> b = lerp_fun(lo,  hi, alpha);
