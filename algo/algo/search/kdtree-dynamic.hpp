...
#ifndef ALGO_SEARCH_KDTREE_DYNAMIC_H_
#define ALGO_SEARCH_KDTREE_DYNAMIC_H_
 ...
 *
 * @note
 * KdTreeDynamic underlying data structure is represented by a binary
 * search tree of nodes with a left and right link and rooted at its
 * parent node.
 * Unlike its static counterpart, the dynamic variant is more flexible
...
public:
    /* --- KdTreeDynamic public API member functions --------------------------
     *
    ...
    void erase(const array_type &array) {
        core_assert(!is_empty(), "kd-tree underflow");
        Point point(array);
        if (!contains(root_, point)) {
            return;
        }
    ...
    /*
     * Return all the points in the kd-tree by running a range
     ...
     *
     * KdTreeDynamic constructor/destructor.
     */
    ...
    ~KdTreeDynamic() { clear(); }
    /*
     * KdTreeDynamic copy constructor/assignment.
     */
...
private:
     ...
     *
     * KdTreeDynamic<Type,Dim>::size
     */
    size_t size(const Node *node) const;
    /*
     * KdTreeDynamic<Type,Dim>::clear
     */
    void clear(Node *node);
    /*
     * KdTreeDynamic<Type,Dim>::insert
     */
    Node *insert(
        Node *node,
        const Point &point,
    ...
    /*
     * KdTreeDynamic<Type,Dim>::sphere_query
     */
    void sphere_query(
        const Node *node,
        const Point &point,
    ...
    /*
     * KdTreeDynamic<Type,Dim>::range_query
     */
    void range_query(
        const Node *node,
        const Rect &rect,
...
KDTREE_T_DECL
inline
void KDTREE_C_DECL::clear(Node *node)
{
    if (node == nullptr) {
        return;
    }
...
/**
 * KdTreeDynamic<Type,Dim>::insert
 * @brief Inserts the given point into the kd-tree.
 * If the kd-tree already contains the specified point,
 * leave the tree unchanged and just return the node.
 */
KDTREE_T_DECL
inline
 ...
 * deletion is modelled after the removal from binary search trees.
...
KDTREE_T_DECL
inline
typename KDTREE_C_DECL::Node *KDTREE_C_DECL::erase(
    Node *node,
    const Point &point)
{
    /*
     ...
     */
    core_assert(node != nullptr, "point does not exist");

    /*
     * If node contains the point, replace it with its successor:
     *
     *  a) if right subtree is not empty, find the minimum along
     *     the current cutting coordinate, and replace the point.
     *  b) otherwise if left subtree is not, empty, find the
     *     minimum along the current cutting coordinate and move
...
/**
 * KdTreeDynamic<Type,Dim>::Point::is_equal
...
/**
 * KdTreeDynamic<Type,Dim>::Point::is_less
...
/**
 * KdTreeDynamic<Type,Dim>::Point::is_greater
...
/**
 * KdTreeDynamic<Type,Dim>::Point::dist
...
/**
 * KdTreeDynamic<Type,Dim>::Rect::contains
...
/**
 * KdTreeDynamic<Type,Dim>::Rect::disjoint
...
/**
 * KdTreeDynamic<Type,Dim>::Rect::dist
 * @brief Return the minimum Euclidean distance from q to any point
 * on this rectangle.
...
/**
 * KdTreeDynamic<Type,Dim>::Rect::left
...
/**
 * KdTreeDynamic<Type,Dim>::Rect::right
...
#endif /* ALGO_SEARCH_KDTREE_DYNAMIC_H_ */
