...
#ifndef TEST_CORE_GEOMETRY_PERFORMANCE_MAT2_H_
#define TEST_CORE_GEOMETRY_PERFORMANCE_MAT2_H_

#include "test-common.hpp"

/** ---------------------------------------------------------------------------
 * test_performance_mat2
 * @brief mat2d performance test.
 */
core_inline_
void test_performance_mat2 (const size_t n_items, const double maxeps)
{
    const size_t dim = 2;
    const core::math::mat2d zero(0.0);
    const core::math::mat2d one(1.0);
    const core::math::mat2d two(2.0);

    /*
     * Create an array of mattors
     */
    core::align_unique_array<core::math::mat2d> a_arr =
        core::make_align_unique_array<core::math::mat2d>(n_items, 1.0);
    core::align_unique_array<core::math::mat2d> b_arr =
        core::make_align_unique_array<core::math::mat2d>(n_items, 2.0);
    core::align_unique_array<core::math::mat2d> c_arr =
        core::make_align_unique_array<core::math::mat2d>(n_items, 0.0);
    ...
    for (size_t ix = 0; ix < n_items; ++ix) {
        core::math::mat2d *a = (*a_arr) + ix;
        core::math::mat2d *b = (*b_arr) + ix;
        core::math::mat2d *c = (*c_arr) + ix;

        size_t id = omp_get_thread_num();
        core::math::RngKiss *kiss = (*kiss_arr) + id;
        core::math::RngDist *rng = (*rng_arr) + id;

        *a = core::math::mat2d((*rng).nrand(*kiss)*rand_sdev + rand_avg,
        ...
        *b = (*a) * (-1.0);
         ...
         */
        *a = core::math::mat2d((*rng).nrand(*kiss)*rand_sdev + 2.0*rand_avg,
                               (*rng).nrand(*kiss)*rand_sdev + rand_avg,
                               (*rng).nrand(*kiss)*rand_sdev + rand_avg,
        ...
        *c = multiply(*a, *b);      /* c = a*a^(-1) */
        ...
        core::math::mat2d ident = identity(*c);
        for (size_t j = 0; j < dim; ++j) {
            for (size_t k = 0; k < dim; ++k) {
        ...
        /* Matrix solve */
        core::math::vec2d vec_b((*rng).nrand(*kiss)*rand_sdev + rand_avg,
                                (*rng).nrand(*kiss)*rand_sdev + rand_avg);
        core::math::mat2d inv_a = inverse(*a);
        /*
         * x = a^-1*b;
         * err = a*x - b
         */
        core::math::vec2d vec_x = multiply(inverse(*a), vec_b);
        core::math::vec2d err = vec_b - multiply(*a, vec_x);
...
#endif /* TEST_CORE_GEOMETRY_PERFORMANCE_MAT2_H_ */
