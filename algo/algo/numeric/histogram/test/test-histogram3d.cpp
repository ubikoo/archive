...
TEST_CASE("Test Histogram3") {
    ...
    SECTION("Kiss uniform distribution") {
         ...
         */
        algo::Histogram3 hist_kiss(numbins3d, numbins3d, numbins3d);
        algo::Histogram3 hist_kiss_tmp(numbins3d, numbins3d, numbins3d);

        hist_kiss.set(0.0, 1.0, 0.0, 1.0, 0.0, 1.0, true);
        for(size_t n = 0; n < numsamples; ++n) {
            hist_kiss.bin(urand(rng_kiss),
                          urand(rng_kiss),
                          urand(rng_kiss),
                          weight);
        }
        ...
        /* Write the histogram */
        filename = TestPrefix + std::string("/out.hist3d_kiss_urand");
        fout.open(filename);
        ...
        REQUIRE((hist_kiss.xbins_ == hist_kiss_tmp.xbins_ &&
                 ...
                 hist_kiss.zbins_ == hist_kiss_tmp.zbins_ &&
                 ...
                 hist_kiss.zmin_  == hist_kiss_tmp.zmin_  &&
                 hist_kiss.zmax_  == hist_kiss_tmp.zmax_));

        /*
         * Kiss normal distribution
         */
        hist_kiss.set(-10.0, 10.0, -10.0, 10.0, -10.0, 10.0, true);
        for(size_t n = 0; n < numsamples; ++n) {
            hist_kiss.bin(nrand(rng_kiss),
                          nrand(rng_kiss),
                          nrand(rng_kiss),
                          weight);
        }
        ...
        /* Write the histogram */
        filename = TestPrefix + std::string("/out.hist3d_kiss_nrand");
        fout.open(filename);
        hist_kiss.write(fout, true);
        fout.close();
    ...
    SECTION("Cmwc uniform distribution") {
         ...
         */
        algo::Histogram3 hist_cmwc(numbins3d, numbins3d, numbins3d);
        algo::Histogram3 hist_cmwc_tmp(numbins3d, numbins3d, numbins3d);

        hist_cmwc.set(0.0, 1.0, 0.0, 1.0, 0.0, 1.0, true);
        for(size_t n = 0; n < numsamples; ++n) {
            hist_cmwc.bin(urand(rng_cmwc),
                          urand(rng_cmwc),
                          urand(rng_cmwc),
                          weight);
        }
        ...
        /* Write the histogram */
        filename = TestPrefix + std::string("/out.hist3d_cmwc_urand");
        fout.open(filename);
        ...
        REQUIRE((hist_cmwc.xbins_ == hist_cmwc_tmp.xbins_ &&
                 ...
                 hist_cmwc.zbins_ == hist_cmwc_tmp.zbins_ &&
                 ...
                 hist_cmwc.zmin_  == hist_cmwc_tmp.zmin_  &&
                 hist_cmwc.zmax_  == hist_cmwc_tmp.zmax_));


        /*
         * Cmwc normal distribution
         */
        hist_cmwc.set(-10.0, 10.0, -10.0, 10.0, -10.0, 10.0, true);
        for(size_t n = 0; n < numsamples; ++n) {
            hist_cmwc.bin(nrand(rng_cmwc),
                          nrand(rng_cmwc),
                          nrand(rng_cmwc),
                          weight);
        }
        ...
        /* Write the histogram */
        filename = TestPrefix + std::string("/out.hist3d_cmwc_nrand");
        fout.open(filename);
        hist_cmwc.write(fout, true);
        fout.close();
