...
class DepthFirstTopoSort:
    ...
    def _dfs(self, graph, s):
        ...
        while not S.is_empty():
            ...
            elif self._state[v] == self.VISITED:
                self._state[v] = self.SORTED
                self._vertices.push(S.pop())
            else:
                _ = S.pop()
...
# -----------------------------------------------------------------------------
def main (argv):
    """ DepthFirstTopoSort test client """
    ...
    toposort = DepthFirstTopoSort(graph)
    print(toposort)
    # draw(toposort, graph)
