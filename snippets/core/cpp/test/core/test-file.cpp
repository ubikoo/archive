...
void test_core_file(void)
{
     ...
     */
    std::cout << "\n>>> Test 1 FileStdIn and FileStdOut\n";
    std::list<std::string> buffer_stdin;
    ...
    try {
        ...
        while (it_stdin != buffer_stdin.end() && it_in != buffer_in.end()) {
            core_assert_(*it_stdin == *it_in, "FAIL");
            ++it_stdin;
            ++it_in;
        }
    } catch (std::exception& e) {
    ...
    try {
        ...
        core_assert_(!fin.is_error(), "I/O error");
        fin.close();
    } catch (std::exception& e) {
        core_throw_(e.what());
    }
    ...
    try {
        ...
        core_assert_(!fout_new.is_error(), "I/O error");
        fout_new.close();

        core_assert_(!fout_csv.is_error(), "I/O error");
        fout_csv.close();
    } catch (std::exception& e) {
        core_throw_(e.what());
    }
    ...
    try {
        ...
        std::cout << core::str::format("file length = %lld bytes\n", length);
        std::cout << core::str::format("buffer size = %lu\n", buffer.size());
        for (char n : buffer) {
            std::cout << n;
        }
        std::cout << "\n";
        ...
        core_assert_(!fout.is_error(), "I/O error");
