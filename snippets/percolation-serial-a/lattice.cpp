 ...
 */
void Lattice::compute(UnionFind &uf) const
{
    uf.clear();
    for (int32_t x = 0; x < m_n_sites; ++x) {
        for (int32_t y = 0; y < m_n_sites; ++y) {
            if (is_open(x, y)) {
                if (is_open(x-1, y)) {
                    uf.join(index(x, y), index(x-1, y));
                }
                if (is_open(x, y-1)) {
                    uf.join(index(x, y), index(x, y-1));
                }
            }
        }
    }
