 ...
 */
core_inline_
void *align_alloc(size_t size, size_t alignment = core_cache_line_size_)
{
    /*
    ...
    core_assert_(ptr != nullptr, "failed to allocate");
 ...
 *
 * @note
 * align_alloc<T> and align_free<T> are paired functions.
 */
 ...
 *
 * @note
 *  align_alloc_array<T> and align_free_array<T> are paired functions.
 */
template<typename T, typename... Args>
 ...
 *
 * @note
 * Ensure the arguments are forwarded correctly to the object constructor
 * in align_alloc.
 *
 * @see
 ...
 *
 * @see
 * https://stackoverflow.com/questions/3582001
 *
 * @note
 *  align_unique_array can be generalized to more than one dimension by
 *  storing a large 1-dimensional block with size(n1*n2*...), and then
 *  having a set of n-1 arrays with offset addresses for each dimension.
 *
 *  This is not implemented because it is more efficient memorywise
 *  to just use a 1-dimensional array of size(n1*n2*n3*...) and use
 *  the appropriate multidimensional addressing of the the form,
 *
...
struct align_unique_array_deleter {
    size_t count_;                          /* Array dimensions. */

    align_unique_array_deleter() : count_(0) {};
    align_unique_array_deleter(const size_t count) : count_(count) {}
    ~align_unique_array_deleter() = default;

    void operator()(T **ptr) {
        align_free_array<T>(*ptr, count_);  /* Free the array */
...
{
    core_assert_(count > 0, "invalid array dimensions");
