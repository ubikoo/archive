 ...
 * and FileOut - provide an abstraction of the C I/O subset of the C++
 * standard library.
 *
 * @note
 * C-streams are objects of type std::FILE that can only be accessed and
 * manipulated through pointers of type std::FILE*.
...
struct File {
    ...
    std::FILE *fp_;     /* pointer to file object */
    bool binary_;       /* is the file mode binary? */
    /*
     * File query functions.
     */
    bool is_open(void) const { return (fp_ != nullptr); }
    bool is_binary(void) const { return binary_; }
     ...
     */
    File() : fp_(nullptr), binary_(false) {}
    virtual ~File() {}
    /*
     * Disable copy constructor/assignment semantics.
     * Each stream is unique to the File object.
     */
 ...
 *  - Read all lines from input stream and returns a list of strings.
...
struct FileStdIn : File {
    ...
    int scanf(const char* format, Args&&... args) const {
        ...
        core_assert_(is_open(), "file stream is not open");
        core_assert_(is_good(), "file stream is not good");
        core_assert_(!is_binary(), "file stream mode is binary");

        return std::fscanf(fp_, format, static_cast<Args&&>(args)...);
    }
     ...
     */
    FileStdIn() {
        fp_ = stdin;        /* set the file stream pointer to stdin */
        binary_ = false;    /* stdin is opened in text mode by default */
    }
    virtual ~FileStdIn() {}
    /*
     * Disable copy constructor/assignment semantics.
...
/**
 * FileStdIn::readlines
 * @brief Read lines from a stream using readline() and return a linked list
...
struct FileIn : FileStdIn {
    ...
    void open(const std::string &filename, const uint32_t mode = Text) {
        /* Assert preconditions. */
        core_assert_(filename.size() > 0, "invalid filename");
        try {
     ...
     */
    FileIn() {
        fp_ = nullptr;      /* fp_ = stdin, reset it by default */
        binary_ = false;    /* reset file mode to text by default */
    }
    FileIn(const char *filename, const uint32_t mode = Text) {
        core_assert_(filename != nullptr, "null filename");
    ...
    FileIn(const std::string &filename, const uint32_t mode = Text) {
        core_assert_(!filename.empty(), "empty filename");
...
void FileIn::open(const char *filename, const uint32_t mode)
{
    /* Assert I/O preconditions. */
    core_assert_(filename != nullptr, "null filename");
    core_assert_(!is_open(), "file stream is already open");

    /* Set file mode and open file stream. */
    binary_ = (mode == Binary);
    std::string modestr = binary_ ? "rb" : "r";

    fp_ = std::fopen(filename, modestr.c_str());
    core_assert_(is_open(), core::str::format(
        "failed to open %s, mode %s\n", filename, modestr.c_str()));
}
...
int64_t FileIn::length(void) const
{
    ...
    core_assert_(is_open(), "file stream is not open");
    core_assert_(is_good(), "file stream is not good");
    core_assert_(is_binary(), "file stream is not binary");

    /* Compute the file length in bytes.  */
    std::fseek(fp_, 0, SEEK_END);   /* go to end of file */
    int64_t len = std::ftell(fp_);  /* get length in bytes */
    std::fseek(fp_, 0, SEEK_SET);   /* go back to begining of file */
    return len;
}
...
bool FileIn::read (void *ptr, size_t size) const
{
    ...
    core_assert_(is_open(), "file stream is not open");
    core_assert_(is_good(), "file stream is not good");
    core_assert_(is_binary(), "file stream mode is not binary");

    core_assert_(ptr != nullptr, "invalid pointer");
    core_assert_(size > 0, "invalid size");

    /* Read one block of size bytes from the file stream. */
    std::fread(ptr, size, 1, fp_);
...
struct FileStdOut : File {
    ...
    bool writelines(const std::list<std::string> &lines,
                     const std::string &sep = std::string()) const;
    /*
    ...
    int printf(const char* format, Args&&... args) const {
        ...
        return std::fprintf(fp_, format, static_cast<Args&&>(args)...);
    }
     ...
     */
    FileStdOut() {
        fp_ = stdout;       /* set the file stream pointer to stdout */
        binary_ = false;    /* stdout is opened in text mode by default */
    }
    virtual ~FileStdOut() {}
    /*
     * Disable copy constructor/assignment semantics.
...
struct FileOut : FileStdOut {
     ...
     */
    FileOut() {
        fp_ = nullptr;      /* fp_ = stdout, reset it by default */
        binary_ = false;    /* reset file mode to text by default */
    }
...
void FileOut::open(const char *filename, const uint32_t mode)
{
    ...
    /* Set file mode and open file stream. */
    binary_ = (mode == Binary);
    std::string modestr = binary_ ? "wb" : "w";
...
int64_t FileOut::length(void) const
{
    ...
    std::fseek(fp_, 0, SEEK_END);   /* go to end of file */
    int64_t len = std::ftell(fp_);  /* get length in bytes */
    std::fseek(fp_, 0, SEEK_CUR);   /* go to current file position */
    return len;
}
