...
class BreadthFirstSearch:
    ...
    compute the set of all vertices reachable from s in the graph using breadth
    first search algorithm.
    Breadth first search algorithm maintains a queue of vertices to be searched
    and eagerly marks each vertex as visited as it is added to the queue.
...
# -----------------------------------------------------------------------------
def main (argv):
    """ BreadthFirstSearch test client """
    ...
    for v in graph.vertices():
        bfs = BreadthFirstSearch(graph, v)
        # draw(bfs, graph)
        print("\nconnected to ", v, ": ", file=sys.stdout, end='')
        print([w for w in graph.vertices() if bfs.is_visited(w)],
            file=sys.stdout, end=' ')
